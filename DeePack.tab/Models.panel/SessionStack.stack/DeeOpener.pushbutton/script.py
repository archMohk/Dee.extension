...
def scan_level(level, project_id, token, max_workers=8):
    ...
    Each folder gets up to _SCAN_MAX_ATTEMPTS attempts, with a backoff that
    waits longer specifically for a 429 rate-limit response, so transient
    API failures don't silently drop entire folders from the results."""
    next_level = []
    found_items = []
    lock = threading.Lock()
    q = queue.Queue()
                ...
                for attempt in range(_SCAN_MAX_ATTEMPTS):
                    try:
                        ...
                        if attempt < _SCAN_MAX_ATTEMPTS - 1:
                            time.sleep(_retry_delay_seconds(attempt, str(e)))  # wait before retry
            finally:
                q.task_done()
